...
const diagnoseMode = false; // Set to true to enable automatic dry-run diagnostics
const localStorageKey = 'multiplicationResults';

// Add these variables at the top of the file if they're not already defined
const masteryThreshold = 90; // percentage
const requiredCorrectAnswers = 3; // per question
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function checkTableMastery() {
        ...
        const totalQuestions = currentTable.questions.length;
        const masteredQuestions = currentTable.questions.filter(q => q.correctCount >= requiredCorrectAnswers).length;
        const masteryPercentage = (masteredQuestions / totalQuestions) * 100;

        console.log(`Checking table mastery for ${currentTable.number}x table:`);
        console.log(`Total questions: ${totalQuestions}`);
        console.log(`Mastered questions: ${masteredQuestions}`);
        console.log(`Mastery percentage: ${masteryPercentage.toFixed(2)}%`);
        console.log(`Mastery threshold: ${masteryThreshold}%`);
        console.log(`Required correct answers per question: ${requiredCorrectAnswers}`);

        currentTable.questions.forEach(q => {
            console.log(`Question ${q.question}: Correct count = ${q.correctCount}`);
        });

        if (masteryPercentage >= masteryThreshold) {
            console.log(`Table ${currentTable.number}x mastered! Moving to next table.`);
            currentTableIndex++;
        ...
        console.log(`Table ${currentTable.number}x not yet mastered. Continuing practice.`);
        return false;
    }
    ...
    function handleSubmit() {
        ...
        checkTableMastery(); // Call this after each answer
    }
